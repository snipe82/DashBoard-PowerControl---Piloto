...
const UsersList = ({ filtros }) => {
  ...
  const handleDesactivarUsuario = async (userId, userEmail) => {
    ...
    if (!window.confirm(`¿Estás seguro de que deseas revocar inmediatamente el acceso al sistema para el usuario ${userEmail}?`)) return;
  ...
  const handleActivarUsuario = async (userId, userEmail) => {
    if (!window.confirm(`¿Deseas restablecer las credenciales y reactivar el acceso al sistema para el usuario ${userEmail}?`)) return;
  ...
  const usuariosFiltrados = usuarios.filter(user => {
    ...
    const nombre = (user.full_name || '').toLowerCase();
    const correo = (user.email || '').toLowerCase();
    return nombre.includes(textoBuscado) || correo.includes(textoBuscado);
  });
        ...
        {!cargando && (
          <div className="flex flex-wrap gap-3 mt-3">
            ...
            <span className="bg-emerald-50 text-emerald-600 border border-emerald-200 px-3 py-1 rounded-lg text-xs font-bold shadow-sm flex items-center gap-1.5">
              ✅ Cuentas Activas: {usuarios.filter(u => u.is_active).length}
            </span>
          </div>
        )}
      </div>
              ...
              {cargando ? (
                ...
                <tr>
                  <td colSpan="4" className="text-center py-12 text-gray-500 italic">
                    {filtros?.busqueda ? 'No se encontraron usuarios que coincidan con la búsqueda.' : 'No se encontraron usuarios registrados.'}
                  </td>
                </tr>
              ) : (
                usuariosFiltrados.map((user, idx) => {
                  ...
                  return (
                    <tr key={user.user_id || idx} className={`transition-all duration-150 ${esCuentaInactiva ? 'bg-gray-50/70 opacity-60' : 'hover:bg-gray-50'}`}>
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold shadow-sm ${esCuentaInactiva ? 'bg-gray-200 text-gray-400' : 'bg-power-purple/10 text-power-purple'}`}>
                      ...
                      <td className="px-6 py-4">
                        <span className={`px-2.5 py-1 rounded border text-[10px] font-bold uppercase tracking-wider ${esCuentaInactiva ? 'bg-gray-100 text-gray-400 border-gray-200' : 'bg-slate-100 text-slate-600 border-slate-200'}`}>
                          {user.role || 'ANALYST'}
                        </span>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex items-center justify-end gap-3.5">
                          <button disabled={esCuentaInactiva} onClick={() => abrirModalEdicion(user)} className="text-power-blue font-bold text-xs hover:underline disabled:opacity-30 disabled:no-underline">Editar Rol</button>
                          {user.is_active ? (
                            <button disabled={user.user_id === currentUserId} onClick={() => handleDesactivarUsuario(user.user_id, user.email)} className="text-rose-500 font-bold text-xs hover:underline disabled:opacity-30 disabled:no-underline" title={user.user_id === currentUserId ? "No puedes auto-desactivarte" : "Desactivar analista"}>Desactivar</button>
                          ) : (
                            <button onClick={() => handleActivarUsuario(user.user_id, user.email)} className="text-emerald-600 font-bold text-xs hover:underline" title="Restablecer accesos al sistema">Activar</button>
                          )}
                        </div>
                      </td>
                    </tr>
